...
**MVP-A fase 1 — core TypeScript implementado** (parser, config, FSM/session, ports, adapters Win/Linux, e2e mockado). Tauri 2 tray stub em `src-tauri/` (single-instance anotado na config; plugin a conectar na integração).
...
- `loadConfig({ appDataDir, readFile? })` — merge `config.default.json` + `~/.config/voz-pc/config.json`
